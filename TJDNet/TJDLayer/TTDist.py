...
class TTDist:
    ...
    def _select_for_batch(
        ...
        Args:
            indices (torch.Tensor): Shape: (n_samples, n_core_repititions).
            alpha (torch.Tensor): Shape: (R)
            beta (torch.Tensor): Shape: (R)
            core (torch.Tensor): Shape: (R, D, R)

        Returns:
            torch.Tensor: Unnormalized probabilities of sequences corresponding to indices. Shape: (n_samples,)
        """

        n_samples = indices.size(0)
        _cores_after_select = []  # [(R, R)] * n_samples * n_core_repititions
        for n in range(n_samples):
            for t in range(n_core_repititions):
                _cores_after_select.append(core[:, indices[n, t], :])

        core_results = []
        for n in range(n_samples):
            core_result = _cores_after_select[n * n_core_repititions]  # (R, R)
            for t in range(1, n_core_repititions):
                core_result = torch.einsum(
                    "ik, kj->ij",
                    core_result,
                    _cores_after_select[n * n_core_repititions + t],
                )
            core_result = torch.einsum(
                "i, ij, j->",
                alpha,
    ...
    def get_prob_and_norm_for_single_batch(
        self, indices: torch.Tensor, batch_idx: int
    ) -> Tuple[Tensor, Tensor]:
        """Get the unnormalized probability and normalization constant of the TTDist.

        Args:
            indices (torch.Tensor): Indices of the sequences. Shape: (n_samples, n_core_repititions).
        ...
        unormalized_probs = self._select_for_batch(
            alpha=self.alpha[batch_idx],
            beta=self.beta[batch_idx],
            core=self.core[batch_idx],
            n_core_repititions=self.n_core_repititions,
            indices=indices,
        )
        check_naninf(unormalized_probs, f"get_prob:unormalized_probs")
    ...
    def materialize(self, normalize: bool = True) -> torch.Tensor:
        ...
        if normalize:
            self.norm_const = (
                ...
                .sum(1)
                .reshape(
                    tuple([batch_size, *[1 for _ in range(self.n_core_repititions)]])
                )
            )
            result = result / self.norm_const
        return result
