...
class ChatTemplateShareGPTV2(BaseChatTemplate):
    TEMPLATE = """[QUESTION]\n{question}\n[ANSWER]\n{answer}"""
    ...
    def format_turn(cls, is_assistant: bool, message: str) -> str:
        ...
        message_end = "###" if is_assistant else ""
        # return prefix + "\n" + message + cls.MESSAGE_END
        return f"{prefix}\n{message}\n{message_end}"
...
def parse_conversation(example, eos_token="<|endoftext|>"):
    ...
    for msg in example["conversations"]:
        ...
        text += ChatTemplateShareGPTV2.format_turn(is_assistant, msg["value"])
        if is_assistant:
            text += eos_token
    return {"text": text}


def load_sharegptv2_data(
    tokenizer,
    input_seq_len,
    test_size=0.01,
