...
def main():
    ...
    true_alpha = torch.randn(1, rank)
    true_beta = torch.randn(1, rank)
    true_core = torch.randn(1, rank, vocab_size, rank)
    true_ttdist = TTDist(
    ...
    alpha = torch.nn.Parameter(torch.randn(1, rank))
    beta = torch.nn.Parameter(torch.randn(1, rank))
    core = torch.nn.Parameter(torch.randn(1, rank, vocab_size, rank))
    ...
    for i in range(n_epochs):
        ...
        print(
            f"[Epoch {i}] Loss: {test_loss.item()} | GT Loss: {test_loss_gt.item()} | Loss Diff (abs): {abs(test_loss.item() - test_loss_gt.item())}"
        )
